...
export async function worktreesMenu(): Promise<void> {
...
async function listWorktrees(): Promise<void> {
  ...
  if (worktrees.length === 0) {
    ...
    for (const wt of worktrees) {
      ...
      console.log(
        `  ${truncate(wt.sessionName, 18).padEnd(20)} ${truncate(wt.branch, 23).padEnd(25)} ${String(wt.stats.commitCount).padEnd(8)} ${changes}`
      );
    }
  }
...
async function createWorktree(): Promise<void> {
  ...
  const sessionName = await input({
    ...
    validate: (val: string) => {
      if (!val.trim()) return "Session name is required";
...
async function selectWorktree(message: string): Promise<WorktreeInfo | null> {
  const mgr = getManager();
  const worktrees = mgr.listWorktrees();
  ...
  if (sessionName === "__cancel__") return null;

  return mgr.getWorktree(sessionName);
}

async function viewWorktree(): Promise<void> {
  const mgr = getManager();
...
async function mergeWorktree(): Promise<void> {
  const mgr = getManager();
  const worktrees = mgr.listWorktrees();
  ...
  if (sessionName === "__cancel__") return;
