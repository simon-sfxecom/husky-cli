...
const STATUS_LABELS: Record<JulesSessionStatus, string> = {
  pending: "Pending",
  planning: "Planning",
...
export async function julesSessionsMenu(): Promise<void> {
...
async function fetchJulesSessions(config: ValidConfig): Promise<JulesSession[]> {
...
async function fetchJulesSources(config: ValidConfig): Promise<{ configured: boolean; sources: JulesSource[] }> {
...
async function selectJulesSession(config: ValidConfig, message: string): Promise<JulesSession | null> {
...
async function listJulesSessions(config: ValidConfig): Promise<void> {
  try {
    const sessions = await fetchJulesSessions(config);
...
async function viewJulesSession(config: ValidConfig): Promise<void> {
  try {
    ...
    console.log(`  Status:           ${STATUS_LABELS[full.status as JulesSessionStatus] || full.status}`);
    ...
    if (full.prUrl) {
      ...
      if (full.prNumber) console.log(`    Number: #${full.prNumber}`);
    }
...
async function createJulesSession(config: ValidConfig): Promise<void> {
  try {
    ...
    console.log(`  Status: ${STATUS_LABELS[session.status as JulesSessionStatus] || session.status}\n`);
    await pressEnterToContinue();
  } catch (error) {
...
async function sendMessage(config: ValidConfig): Promise<void> {
  try {
    ...
    console.log(`  Status: ${STATUS_LABELS[updated.status as JulesSessionStatus] || updated.status}\n`);
    await pressEnterToContinue();
  } catch (error) {
...
async function approvePlan(config: ValidConfig): Promise<void> {
  try {
    ...
    if (sessionId === "__cancel__") return;
...
async function viewActivities(config: ValidConfig): Promise<void> {
  try {
...
async function viewSources(config: ValidConfig): Promise<void> {
  try {
...
async function deleteJulesSession(config: ValidConfig): Promise<void> {
  try {
