...
interface TargetAudiencePersona {
  id: string;
  name: string;
  description: string;
...
export async function strategyMenu(): Promise<void> {
...
async function showStrategy(config: ValidConfig): Promise<void> {
  try {
    ...
    if (strategy.values.length === 0) {
      ...
      for (const v of strategy.values) {
        console.log(`  - ${v.name}`);
        if (v.description) console.log(`    ${truncate(v.description, 50)}`);
      }
    }
    ...
    if (strategy.targetAudience.length === 0) {
      ...
      for (const p of strategy.targetAudience) {
        console.log(`  - ${p.name}`);
        if (p.description) console.log(`    ${truncate(p.description, 50)}`);
      }
    }
...
async function setVision(config: ValidConfig): Promise<void> {
  try {
    const strategy = await fetchStrategy(config);
...
async function setMission(config: ValidConfig): Promise<void> {
  try {
    const strategy = await fetchStrategy(config);
...
// VALUES SUB-MENU
async function valuesSubMenu(config: ValidConfig): Promise<void> {
  const menuItems: MenuItem[] = [
...
async function listValues(config: ValidConfig): Promise<void> {
  const strategy = await fetchStrategy(config);
  if (!strategy) {
...
async function addValue(config: ValidConfig): Promise<void> {
  try {
...
async function updateValue(config: ValidConfig): Promise<void> {
  try {
    const strategy = await fetchStrategy(config);
...
async function deleteValue(config: ValidConfig): Promise<void> {
  try {
    const strategy = await fetchStrategy(config);
    if (!strategy || strategy.values.length === 0) {
      console.log("\n  No values to delete.\n");
...
// GOALS SUB-MENU
async function goalsSubMenu(config: ValidConfig): Promise<void> {
  const menuItems: MenuItem[] = [
...
async function listGoals(config: ValidConfig): Promise<void> {
  const strategy = await fetchStrategy(config);
  if (!strategy) {
    console.log("\n  Could not fetch strategy.\n");
    await pressEnterToContinue();
  ...
  if (strategy.goals.length === 0) {
    ...
    for (const g of strategy.goals) {
      ...
      if (g.dueDate) console.log(`    Due: ${new Date(g.dueDate).toLocaleDateString()}`);
      console.log(`    ID: ${g.id}`);
    }
  }
...
async function addGoal(config: ValidConfig): Promise<void> {
  try {
...
async function updateGoal(config: ValidConfig): Promise<void> {
  try {
    const strategy = await fetchStrategy(config);
...
async function deleteGoal(config: ValidConfig): Promise<void> {
  try {
    const strategy = await fetchStrategy(config);
    if (!strategy || strategy.goals.length === 0) {
      console.log("\n  No goals to delete.\n");
...
// PERSONAS SUB-MENU
async function personasSubMenu(config: ValidConfig): Promise<void> {
  const menuItems: MenuItem[] = [
...
async function listPersonas(config: ValidConfig): Promise<void> {
  const strategy = await fetchStrategy(config);
  if (!strategy) {
    console.log("\n  Could not fetch strategy.\n");
    await pressEnterToContinue();
...
async function addPersona(config: ValidConfig): Promise<void> {
  try {
    const name = await input({
...
async function updatePersona(config: ValidConfig): Promise<void> {
  try {
    const strategy = await fetchStrategy(config);
    ...
    if (personaId === "__cancel__") return;
...
async function deletePersona(config: ValidConfig): Promise<void> {
  try {
    const strategy = await fetchStrategy(config);
    if (!strategy || strategy.targetAudience.length === 0) {
