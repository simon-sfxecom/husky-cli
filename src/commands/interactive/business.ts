...
import { MenuItem, pressEnterToContinue } from "./utils.js";
...
async function ticketsSubMenu(): Promise<void> {
    const menuItems: MenuItem[] = [
        ...
        switch (choice) {
            ...
            case "knowledge":
                ...
                for (const r of kResults) {
                    const p = r.payload as Record<string, unknown>;
                    console.log(`  [${(r.score * 100).toFixed(1)}%] ${p?.subject || r.id}`);
                    if (p?.resolution) console.log(`    → ${String(p.resolution).slice(0, 60)}...`);
                }
                break;
...
async function customersSubMenu(): Promise<void> {
    const menuItems: MenuItem[] = [
...
async function seatableSubMenu(): Promise<void> {
    const menuItems: MenuItem[] = [
        ...
        switch (choice) {
            ...
            case "supplier":
                ...
                const filtered = allRows.filter(r =>
                    String(r.Name || "").toLowerCase().includes(lq) ||
...
async function qdrantSubMenu(): Promise<void> {
    const menuItems: MenuItem[] = [
