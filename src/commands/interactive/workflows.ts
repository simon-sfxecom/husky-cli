...
export async function workflowsMenu(): Promise<void> {
...
async function selectWorkflow(config: ValidConfig, message: string): Promise<Workflow | null> {
  ...
  if (workflowId === "__cancel__") return null;
...
async function viewWorkflow(config: ValidConfig): Promise<void> {
  try {
...
async function updateWorkflow(config: ValidConfig): Promise<void> {
  try {
...
async function deleteWorkflow(config: ValidConfig): Promise<void> {
  try {
...
async function manageSteps(config: ValidConfig): Promise<void> {
  try {
    const wf = await selectWorkflow(config, "Select workflow:");
    if (!wf) return;

    const stepChoices: MenuItem[] = [
    ...
    if (action === "list") {
      ...
      const stepChoices = fullWf.steps.map((s: WorkflowStep) => ({
...
async function generateSteps(config: ValidConfig): Promise<void> {
  try {
    const wf = await selectWorkflow(config, "Select workflow:");
    if (!wf) return;
