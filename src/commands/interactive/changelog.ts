...
export async function changelogMenu(): Promise<void> {
...
async function fetchChangelogs(config: ValidConfig, projectId?: string): Promise<Changelog[]> {
...
async function selectChangelog(config: ValidConfig, message: string): Promise<Changelog | null> {
  ...
  if (changelogId === "__cancel__") return null;
...
async function listChangelogs(config: ValidConfig): Promise<void> {
  try {
    const changelogs = await fetchChangelogs(config);
...
async function viewChangelog(config: ValidConfig): Promise<void> {
  try {
    ...
    if (changelog.entries.length > 0) {
      // Group by type
      const byType: Record<string, typeof changelog.entries> = {};
...
async function generateChangelog(config: ValidConfig): Promise<void> {
  try {
    // Select project
    ...
    if (projectId === "__cancel__") return;
...
async function publishChangelog(config: ValidConfig): Promise<void> {
  try {
    const changelogs = await fetchChangelogs(config);
    ...
    if (changelogId === "__cancel__") return;
...
async function deleteChangelog(config: ValidConfig): Promise<void> {
  try {
...
// Git helpers
function getGitCommits(since?: string, until?: string): GitCommit[] {
  try {
    let range = "";
    if (since && until) {
      range = `${since}..${until}`;
...
function parseCommitLine(line: string, commits: GitCommit[]): void {
