    ...
    .action(async (email, options) => {
        try {
            ...
            if (addr) {
                ...
                if (addr.Company) console.log(`  Company: ${addr.Company}`);
    ...
    .action(async (email, options) => {
        try {
            ...
            if (result.billbee) {
                console.log(`\n  📦 ORDERS (Billbee)`);
                console.log("  " + "─".repeat(40));
                if (result.billbee.name) console.log(`  Name:    ${result.billbee.name}`);
                if (result.billbee.company) console.log(`  Company: ${result.billbee.company}`);
                if (result.billbee.phone) console.log(`  Phone:   ${result.billbee.phone}`);
