...
import { BillbeeClient, OrderState, OrderStateLabels } from "../../lib/biz/index.js";
...
ordersCommand
    ...
    .action(async (options) => {
        try {
            ...
            let orderStateId: number[] | undefined;
            if (options.status) {
                const statusLower = options.status.toLowerCase();
                const stateEntry = Object.entries(OrderStateLabels).find(
                    ([, label]) => label === statusLower
                );
                if (stateEntry) {
    ...
    .action(async (id, options) => {
        try {
            ...
            if (addr) {
                ...
                if (addr.Email) console.log(`    ${addr.Email}`);
            }
...
ordersCommand
    ...
    .action(async (id, options) => {
        try {
            ...
            if (options.state) {
                const stateEntry = Object.entries(OrderStateLabels).find(
                    ([, label]) => label === options.state.toLowerCase()
                );
                if (stateEntry) {
