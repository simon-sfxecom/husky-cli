...
function getGitCommits(since?: string, until?: string): Array<{
  hash: string;
  shortHash: string;
  message: string;
  body: string;
...
changelogCommand
  ...
  .action(async (options) => {
    ...
    try {
      ...
      const changelogs = await res.json() as Changelog[];
      ...
      for (const changelog of changelogs) {
        ...
        console.log(
          `  ${statusIcon} ${changelog.version.padEnd(15)} ${date.padEnd(12)} ${changelog.entries.length} entries`
        );
      }
      console.log("");
    } catch (error) {
...
function formatAsMarkdown(changelog: Changelog): string {
