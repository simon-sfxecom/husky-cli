...
function printSingleSetting(key: string, data: Setting | { value: string | number | boolean | object }) {
...
function printAllSettings(data: Record<string, Setting | string | number | boolean | object> | Setting[]) {
  // Handle both array and object formats
  const entries: Array<{ key: string; value: string | number | boolean | object }> = [];
  ...
  if (Array.isArray(data)) {
    ...
    for (const [key, value] of Object.entries(data)) {
      if (typeof value === "object" && value !== null && "value" in value) {
        entries.push({ key, value: (value as Setting).value });
      } else {
        entries.push({ key, value: value as string | number | boolean | object });
      }
    }
  }
  ...
  for (const entry of entries) {
    ...
    console.log(
      `  ${entry.key.padEnd(30)} ${truncatedValue}`
    );
  }
