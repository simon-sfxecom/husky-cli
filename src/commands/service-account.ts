...
function formatPersona(personaType: string): string {
...
serviceAccountCommand
  ...
  .action(async (options: { json?: boolean }) => {
    try {
      ...
      const bindings: ServiceAccountBinding[] = (data.bindings || []) as ServiceAccountBinding[];
...
serviceAccountCommand
  ...
  .option("--json", "Output as JSON")
  .action(async (persona: string, options: { json?: boolean }) => {
    try {
...
serviceAccountCommand
  ...
  .action(async () => {
    try {
      ...
      for (const result of (data.results || []) as SyncResult[]) {
