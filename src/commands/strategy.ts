...
strategyCommand
  ...
  .action(async (options) => {
    ...
    try {
      ...
      const strategy: BusinessStrategy = await res.json();
...
strategyCommand
  ...
  .action(async (options) => {
    ...
    const characteristics = options.characteristics
      ? options.characteristics.split(",").map((c: string) => c.trim()).filter((c: string) => c)
      : [];
...
// Print functions
function printStrategy(strategy: BusinessStrategy) {
  console.log("\n  BUSINESS STRATEGY");
...
function getGoalStatusIcon(status: GoalStatus): string {
  switch (status) {
    case "not_started":
      return "[ ]";
    case "in_progress":
